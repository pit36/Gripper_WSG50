...
{
    cout << ("WSG50Communicator(): IP = ") << ip << ", PORT = " << port;
    // set ip and port
    //
    this->_IP = ip;
    this->_PORT = port;
...
void WSG50Communicator::startConnection(void)
{
    ...
    if(_connection != nullptr) {
        if(DEBUG) {
            cout << ("It seems that already a connection is established!");
        }
        return;
    }
...
{
    if(!this->_keep_alive) {
        cout << ("connection seems already closed.");
        return;
    }
...
//    if(DEBUG) cout << ("read_handler called... (length: %d)", (int) len);
...
cout << "4: " << ec << std::endl;
        ...
        if(pos1 != 0) {
            ...
            if(_iMsgBufferSize > 0) {
                ...
                if(DEBUG) {
                    cout << ("Message has been split. try concatenate messages");
                ...
                if(DEBUG) cout << ("missing the first part of the message!");
            }
        }
...
//            cout << ("response message length = %d", responseMsg.length);
            ...
            if(responseMsg.length == -1) {
                cout << ("could not create TRESPONSE. continue");
            ...
            if(responseMsg.id == 0x07) {
                if(responseMsg.status_code == E_SUCCESS) {
                    cout << ("Received successfull disconnect response!\n");
                    io_service.stop();
                } else {
                    printErrorCode(responseMsg.status_code);
                }
...
{
//    if(DEBUG) cout << ("connect_handler called...");
...
{
//    if(DEBUG) cout << ("resolve_handler called...");

    if (!ec)
    {
        // call connection handler
        //
        sock.async_connect(*it, boost::bind(&WSG50Communicator::connect_handler, this, boost::asio::placeholders::error));
    }
}
...
void WSG50Communicator::connect()
{
    ...
    resolver.async_resolve(query, boost::bind(&WSG50Communicator::resolve_handler, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
...
void WSG50Communicator::printErrorCode(TStat ec)
{
    ...
    case E_NOT_AVAILABLE:
        cout << ("Received command error code: E_NOT_AVAILABLE");
        break;
    case E_NO_SENSOR:
        cout << ("Received command error code: E_NO_SENSOR");
        break;
    case E_NOT_INITIALIZED:
        cout << ("Received command error code: E_NOT_INITIALIZED");
        break;
    case E_ALREADY_RUNNING:
        cout << ("Received command error code: E_ALREADY_RUNNING");
        break;
    case E_FEATURE_NOT_SUPPORTED:
        cout << ("Received command error code: E_FEATURE_NOT_SUPPORTED");
        break;
    case E_INCONSISTENT_DATA:
        cout << ("Received command error code: E_INCONSISTENT_DATA");
        break;
    case E_TIMEOUT:
        cout << ("Received command error code: E_TIMEOUT");
        break;
    case E_READ_ERROR:
        cout << ("Received command error code: E_READ_ERROR");
        break;
    case E_WRITE_ERROR:
        cout << ("Received command error code: E_WRITE_ERROR");
        break;
    case E_INSUFFICIENT_RESOURCES:
        cout << ("Received command error code: E_INSUFFICIENT_RESOURCES");
        break;
    case E_CHECKSUM_ERROR:
        cout << ("Received command error code: E_CHECKSUM_ERROR");
        break;
    case E_NO_PARAM_EXPECTED:
        cout << ("Received command error code: E_NO_PARAM_EXPECTED");
        break;
    case E_NOT_ENOUGH_PARAMS:
        cout << ("Received command error code: E_NOT_ENOUGH_PARAMS");
        break;
    case E_CMD_UNKNOWN:
        cout << ("Received command error code: E_CMD_UNKNOWN");
        break;
    case E_CMD_FORMAT_ERROR:
        cout << ("Received command error code: E_CMD_FORMAT_ERROR");
        break;
    case E_ACCESS_DENIED:
        cout << ("Received command error code: E_ACCESS_DENIED");
        break;
    case E_ALREADY_OPEN:
        cout << ("Received command error code: E_ALREADY_OPEN");
        break;
    case E_CMD_FAILED:
        cout << ("Received command error code: E_CMD_FAILED");
        break;
    case E_CMD_ABORTED:
        cout << ("Received command error code: E_CMD_ABORTED");
        break;
    case E_INVALID_HANDLE:
        cout << ("Received command error code: E_INVALID_HANDLE");
        break;
    case E_NOT_FOUND:
        cout << ("Received command error code: E_NOT_FOUND");
        break;
    case E_NOT_OPEN:
        cout << ("Received command error code: E_NOT_OPEN");
        break;
    case E_IO_ERROR:
        cout << ("Received command error code: E_IO_ERROR");
        break;
    case E_INVALID_PARAMETER:
        cout << ("Received command error code: E_INVALID_PARAMETER");
        break;
    case E_INDEX_OUT_OF_BOUNDS:
        cout << ("Received command error code: E_INDEX_OUT_OF_BOUNDS");
        break;
    case E_CMD_PENDING:
        cout << ("Received command error code: E_CMD_PENDING");
        break;
    case E_OVERRUN:
        cout << ("Received command error code: E_OVERRUN");
        break;
    case E_RANGE_ERROR:
        cout << ("Received command error code: E_RANGE_ERROR");
        break;
    case E_AXIS_BLOCKED:
        cout << ("Received command error code: E_AXIS_BLOCKED");
        break;
    case E_FILE_EXISTS:
        cout << ("Received command error code: E_FILE_EXISTS");
        break;
    default:
        break;
    }
...
bool WSG50Communicator::msg_send(TMESSAGE * msg )
{
    ...
    // Transmit buffer:
    //
//    if(DEBUG) cout << ("Send tcp message.");
    boost::asio::write(sock, boost::asio::buffer(WSGBUF, WSGSIZE));
...
TRESPONSE WSG50Communicator::createTRESPONSE(unsigned char * data, size_t TCPPacketLength)
{
    ...
    if(TCPPacketLength < 8) {
        ...
        if(DEBUG) cout << ("response message incomplete! ignoring message");
        respMsg.length = -1;
        return respMsg;
    }
    ...
    {
        cout << ("1 Length of response TCP-message does not match announced data-length!");
        cout << ("2 actual length = %d, announced data-length = %d", ((int) TCPPacketLength -10), (int) respMsg.length);

...
void WSG50Controller::updateHandler(void)
{
    ...
    case 0x20:  // Homing
        if(resp.status_code == E_CMD_PENDING) {
            if(DEBUG) std::cout << ("Do homing...");
            _ready = false;
        } else if(resp.status_code == E_SUCCESS) {
            _ready = true;
            if(DEBUG) std::cout << ("Reached homing position.");
        } else {
            std::cout << ("Some error occured during homing:");
    ...
    case 0x21:  // Pre-Position Fingers
        if(resp.status_code == E_SUCCESS) {
            _ready = true;
            std::cout << ("Reached preposition fingers position. ");
        } else if(resp.status_code == E_AXIS_BLOCKED) {
            std::cout << ("Axis blocked. Stopping motion.");
            _ready = true;
        } else if(resp.status_code == E_CMD_ABORTED) {
            std::cout << ("STOP command has been issued while Pre-Positioning fingers.");
            _ready = true;
        } else if(resp.status_code == E_CMD_PENDING) {
            std::cout << ("Pre-Position Fingers: Command Pending");
            _ready = false;
        } else {
            std::cout << ("Pre-Position Fingers:");
            _wsgComm->printErrorCode(resp.status_code);
            _ready = true;
        }
        break;
    case 0x22:  // STOP Command
        if(resp.status_code == E_SUCCESS) {
            std::cout << ("STOP command successfull");
            _ready = true;
        } else if(resp.status_code == E_NO_PARAM_EXPECTED) {
            std::cout << ("STOP Command: No parameters expected!");
        } else if(resp.status_code == E_TIMEOUT) {
            std::cout << ("STOP Command: timeout occured! Could not stop the gripper.");
        }
        break;
    case 0x23:  // FAST STOP Command
        if(resp.status_code == E_SUCCESS) {
            std::cout << ("FAST STOP has been issued successfully. Preventing any further motion until acknowledgement message.");
            _ready = false;
        } else {
            _wsgComm->printErrorCode(resp.status_code);
            _ready = false;
    ...
    case 0x25:  // _Grasping
        if(resp.status_code == E_SUCCESS) {
            std::cout << ("Grasped part. What to do next?");
            _ready = true;
        } else if(resp.status_code == E_CMD_PENDING
                  || resp.status_code == E_ALREADY_RUNNING) {
            std::cout << ("Currently busy grasping...");
            _ready = false;
        } else {
            _wsgComm->printErrorCode(resp.status_code);
            _ready = true;
    ...
    case 0x26:  // Release part
        if(resp.status_code == E_SUCCESS) {
            std::cout << ("Part has been dropped. What to do next?");
            _ready = true;
        } else if(resp.status_code == E_CMD_PENDING) {
            std::cout << ("Release Part: Command Pending.");
            _ready = false;
        } else if(resp.status_code == E_ALREADY_RUNNING) {
            std::cout << ("Release Part: Error already running");
            _ready = false;
        } else {
            _wsgComm->printErrorCode(resp.status_code);
            _ready = true;
    ...
    case 0x30:  // _SETACC (Set Acceleration)
        if(resp.status_code == E_SUCCESS) {
            if(DEBUG) std::cout << ("Acceleration set.");
        } else {
            _wsgComm->printErrorCode(resp.status_code);
        }
        _ready = true;
...
//            printf("Acceleration Value: %f\n\n", test);
    ...
    case 0x34:  // Set Soft Limits
        if(resp.status_code == E_SUCCESS) {
            std::cout << ("Soft Limits have been set.");
            _softLimitsSet = true;
        } else {
            _wsgComm->printErrorCode(resp.status_code);
    ...
    case 0x36:  // Clear Soft Limits
        if(resp.status_code == E_SUCCESS) {
            std::cout << ("soft limits cleared successfully.");
            _softLimitsSet = false;
    ...
    case 0x38:  // Tare Force Sensor
        if(resp.status_code == E_SUCCESS) {
            ...
            std::cout << ("TARE FORCE SENSOR response: Command is not allowed in force control mode!");
            _systStatesReadyForCommand = true;
        } else if(resp.status_code == E_NO_PARAM_EXPECTED) {
    ...
    case 0x41:  // get Grasping State
        if(resp.status_code == E_SUCCESS) {
            ...
            std::cout << ("GET GRASPING STATE response: Unexpected Error: Command length mismatch");
            _wsgComm->printErrorCode(resp.status_code);
            _systStatesReadyForCommand = true;
        }
        break;
    case 0x43:  // get opening width
        if(resp.status_code == E_SUCCESS) {
            // std::cout << ("GET OPENING WIDTH response: Success!");
            if(resp.length == 4) {
                _currentWidthMutex.lock();
                memcpy(&_currentOpeningWidth, resp.data, sizeof(float));
                _currentWidthMutex.unlock();
//                if(DEBUG) std::cout << ("current opening width: %f", _currentOpeningWidth);
            } else {
    ...
    case 0x44:  // get speed
        if(resp.status_code == E_SUCCESS) {
            // std::cout << ("GET SPEED response: Success!");
            if(resp.length == 4) {
                _currentSpeedMutex.lock();
                memcpy(&_currentSpeed, resp.data, sizeof(float));
                _currentSpeedMutex.unlock();
//                if(DEBUG) std::cout << ("current speed: %f", _currentSpeed);
            } else {
                std::cout << ("expected 4 byte of data for opening width!");
            }
            _systStatesReadyForCommand = true;
        } else if(resp.status_code == E_CMD_FORMAT_ERROR) {
            std::cout << ("GET SPEED response: Command length mismatch");
            _systStatesReadyForCommand = true;
        } else {
            std::cout << ("GET SPEED response: Unexpected Error:");
            _wsgComm->printErrorCode(resp.status_code);
            _systStatesReadyForCommand = true;
        }
        break;
    case 0x45:  // get Force
        if(resp.status_code == E_SUCCESS) {
            // std::cout << ("GET FORCE response: Success!");
            if(resp.length == 4) {
                _currentForceMutex.lock();
                memcpy(&_currentForce, resp.data, sizeof(float));
                _currentForceMutex.unlock();
//                if(DEBUG) std::cout << ("current force: %f", _currentForce);
            } else {
                std::cout << ("expected 4 byte of data for opening width!");
            }
            _systStatesReadyForCommand = true;
        } else if(resp.status_code == E_CMD_FORMAT_ERROR) {
            std::cout << ("GET FORCE response: Command length mismatch");
            _systStatesReadyForCommand = true;
        } else {
            std::cout << ("GET FORCE response: Unexpected Error:");
            _wsgComm->printErrorCode(resp.status_code);
            _systStatesReadyForCommand = true;
        }
        break;
...
void WSG50Controller::prePositionFingers(bool stopOnBlock, float width, float speed)
{
    ...
    //
    if(!_ready) {
        std::cout << ("prePositionFingers(): Gripper is not ready to receive another command!");
        return;
    }
    // prevent from sending other messages
    //
    ...
    if(DEBUG) {
        std::cout << ("data package for preposition fingers: ");
...
void WSG50Controller::setAcceleration(float acceleration)
{
    ...
    //
    if(!_ready) {
        std::cout << ("setAcceleration(): Gripper is not ready to receive another command!");
        return;
    }
    // prevent from sending other messages
    //
    ...
    if(DEBUG) std::cout << ("Set Acceleration: %f\n", this->_acceleration);
...
void WSG50Controller::setForceLimit(float forcelimit)
{
    ...
    //
    if(!_ready) {
        std::cout << ("setForceLimit(): Gripper is not ready to receive another command!");
        return;
    }
    // prevent from sending other messages
    //
    ...
    if(DEBUG) std::cout << ("Set ForceLimit: %f\n", this->_forceLimit);
...
void WSG50Controller::setSoftLimits(float minusLimit, float plusLimit)
{
    ...
    // check if ready
    if(!_ready) {
        std::cout << ("setSoftLimits(): Gripper is not ready to receive another command!");
        return;
    }
    // prevent from sending other messages
    //
    ...
    if(minusLimit >= _MaxWidth) {
        std::cout << ("minusLimit is too high!");
        return;
    }
    if(plusLimit <= _MinWidth) {
        std::cout << ("plusLimit is too low!");
    }
...
void WSG50Controller::clearSoftLimits()
{
    ...
    //
    if(!_ready) {
        std::cout << ("clearSoftLimits(): Gripper is not ready to receive another command!");
        return;
    }
    // prevent from sending other messages
    //
...
void WSG50Controller::tareForceSensor()
{
    ...
    //
    if(!_ready) {
        std::cout << ("tareForceSensor(): Gripper is not ready to receive another command!");
        return;
    }
    // prevent from sending other messages
    //
...
{
    if(DEBUG) std::cout << ("Set auto-updates for opening width!");
...
void WSG50Controller::getForceUpdates(bool updateOnChangeOnly,
                                      ...
                                      short updatePeriodInMs)
{
    if(DEBUG) std::cout << ("Set auto-updates for force!");
...
void WSG50Controller::getSpeedUpdates(bool updateOnChangeOnly,
                                      ...
                                      short updatePeriodInMs)
{
    if(DEBUG) std::cout << ("Set auto-updates for speed!");
...
void WSG50Controller::getGraspingStateUpdates(bool updateOnChangeOnly,
                                              ...
                                              short updatePeriodInMillisec)
{
    if(DEBUG) std::cout << ("Set auto-updates for grasping state!");
